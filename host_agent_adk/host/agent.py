...
logging.getLogger("fast_api").disabled = True
logging.getLogger("google_llm").disabled = True
...
class HostAgent:
    ...
    async def send_message(self, agent_name: str, task: str, tool_context: ToolContext):
        ...
        # The remote client returns an async generator of events; consume it
        final_task: Task | None = None
        async for event in client.send_message(message_request):
            root = getattr(event, "root", event)
            payload = getattr(root, "result", root)
            ...
            if isinstance(payload, TaskStatusUpdateEvent):
                ...
                if msg and msg.parts:
                    ...
                    for part in msg.parts:
                        # Prefer direct text; fallback to root.text; then dict
                        txt = getattr(part, "text", None)
                        if txt is None:
                            root_part = getattr(part, "root", None)
                            txt = getattr(root_part, "text", None)
                        if txt is None and hasattr(part, "model_dump"):
                            d = part.model_dump(exclude_none=True)
                            txt = d.get("text") or (d.get("root") or {}).get("text")
                        if txt:
                            texts.append(txt)
                    print("status update text:", " ".join(texts))
            if artifact:
                final_task = event
            # elif isinstance(event, (TaskArtifactUpdateEvent, TaskStatusUpdateEvent)):
            #     # Ignored for now; could be used for progressive updates
            #     pass

        if final_task is None:
            print("Did not receive a final Task from the remote agent stream.")
            return []

        # Extract artifact parts from the final task
        try:
            response_content = final_task.root.model_dump_json(exclude_none=True)
            json_content = json.loads(response_content)
        except Exception:
            print("Failed to serialize final Task; returning empty response.")
            return []

        print("json_content", json_content)
        resp = []
        if json_content.get("result", {}).get("artifact"):
            artifact = json_content["result"]["artifact"]
